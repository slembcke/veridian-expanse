...
float4 FShader(in DriftSpriteFragInput FRAG) : SV_TARGET0 {
	...
	// TODO aspect
	// float2 reflect_offset = -0.25*deriv/dot(deriv, deriv);
	// float3 reflect = DriftPrevFrame.Sample(DriftNearest, FRAG.ssuv_prev + reflect_offset).rgb;
	// reflect *= 0.3*smoothstep(0, 3, dot(deriv, deriv));
	
	float3 light = SampleLightField(normalize(n.xyz), FRAG.ssuv, 1) + props.b;
